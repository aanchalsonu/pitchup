...
#     if request.method == 'POST':
#         form = forms.EditProfileForm(request.POST, request.FILES, instance=request.user.profile)
#         if form.is_valid():
#             form.save()
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if 'picture' in request.FILES:
                profile_picture = Image.open(profile.picture)
                ...
                image_io = BytesIO()
                profile_picture.save(image_io, format='JPEG', quality=90)
                image_io.seek(0)

                # Save the image to the ImageField
                profile.picture.save('profile.jpg', ContentFile(image_io.read()))
